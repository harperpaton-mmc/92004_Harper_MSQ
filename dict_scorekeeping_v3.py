...
# Test Class (mind the mess)
class Counter: # This class is used to calculate the user's accuracy with that question type.
    # The program should be able to find how many times 'self' (the question type) appears and is answered correctly by checking the history lists
    # It should then use those values to return "(correct answers)/(total questions of that type)"
    # E.x. "Gravitational Energy: 4 / 10"
    def __init__(self, type):
        self.type = type # Self attaches the characteristic 'type' to the objects of the class
    def accuracy(self): # Finding the values
        correct_amount = answer_history.count(self.type)
        total_amount = question_history.count(self.type)
...
while rounds_played < num_rounds:
    ...
    print() # Loop ends (repeats here

# For testing

# Calculating statistics (inefficient but works)
# Using a dictionary to store the data without the class (each item must be calculated separately)
score_dict = dict(kinetic = f"{answer_history.count('kinetic energy')} / {question_history.count('kinetic energy')}", gravitational =f"{answer_history.count('gravitational energy')} / {question_history.count('gravitational energy')}",
                  work = f"{answer_history.count('work')} / {question_history.count('work')}", power =f"{answer_history.count('power')} / {question_history.count('power')}", voltage =f"{answer_history.count('voltage')} / {question_history.count('voltage')}",
                  circuit_power = f"{answer_history.count('circuit power')} / {question_history.count('circuit power')}", heat_increase =  f"{answer_history.count('heat increase')} / {question_history.count('heat increase')}", heat_change = f"{answer_history.count('heat change')} / {question_history.count('heat change')}")

print(score_dict)

# Separate dictionary to test if my classes are working - this should return the same data as score_dict, but is written nicer
# Generating class objects
kin = Counter("kinetic energy")
gra = Counter("gravitational energy")
work = Counter("work")
powr = Counter("power")
volt = Counter("voltage")
circ = Counter("circuit power")
h_inc = Counter("heat increase")
h_cha = Counter("heat change")

score_dict_alt = dict(Kinetic = kin.accuracy(), Gravitational = gra.accuracy(), Work = work.accuracy(), Power = powr.accuracy(), Voltage = volt.accuracy(), CircutPower = circ.accuracy(), HeatIncrease = h_inc.accuracy(), HeatChange = h_cha.accuracy())
print(score_dict_alt)
