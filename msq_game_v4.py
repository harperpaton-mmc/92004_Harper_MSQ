...
def yes_no_checker(question, valid_ans=('yes', 'no')):
    error = f"[[Error: Please enter a valid option from the following list: {valid_ans}!]]"
...
Extra Tips
────────────────────
Round all answers up to a whole number.
...
def questions_checker(question):
   while True:
       ...
       error = "[[Error: Please enter an integer between 1 and 50!]]"
...
# Calculates and displays the user's statistics so they can see what areas they perform better at and the areas they may need to practice
def display_statistics():
    ...
    # Dictionary to store the counter instances
    score_dict = dict(Kinetic=kin.accuracy(), Gravitational=gra.accuracy(), Work=work.accuracy(), Power=powr.accuracy(),
                      Voltage=volt.accuracy(), CircutPower=circ.accuracy(), HeatIncrease=h_inc.accuracy(),
                      HeatChange=h_cha.accuracy())
    # Keeps only the question types where that question has been asked
    displayed_scores = {key: value for key, value in score_dict.items() if value != "N/A"}
...
class Counter: # This class is used to calculate the user's accuracy with that question type.
    ...
    def accuracy(self): # Finding the values
        ...
        # Helps remove questions that have not been asked from statistics later
        if total_amount == 0:
            return "N/A"
        else:
